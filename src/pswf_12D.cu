...
__host__ void PSWF_12D::buildGCF(float amp, float x0, float y0, float sigma_x, float sigma_y)
{
    ...
    for(int i=0; i<this->m; i++) {
        for(int j=0; j<this->n; j++) {
            y = (i-this->support_y)*sigma_y;
            x = (j-this->support_x)*sigma_x;
            val = GCF(amp, x, y, x0, y0, sigma_x, sigma_y, this->w1);
            this->kernel[this->n * i + j] = val;
        }
    }
    this->copyKerneltoGPU();
...
__host__ float PSWF_12D::GCF(float amp, float x, float y, float x0, float y0, float sigma_x, float sigma_y, float w)
{
    GCF(amp, x, y, x0, y0, sigma_x, sigma_y, w, NULL);
}
