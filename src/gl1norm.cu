...
GL1Norm::GL1Norm(float *prior, float normalization_factor, float epsilon_a,
                 float epsilon_b) {
  this->name = "G L1-Norm";
  this->prior = prior;
  this->epsilon_a = epsilon_a;
...
GL1Norm::GL1Norm(std::vector<float> prior, float normalization_factor,
                 float epsilon_a, float epsilon_b) {
  this->name = "G L1-Norm";
  this->epsilon_a = epsilon_a;
  this->epsilon_b = epsilon_b;
  this->normalization_factor = normalization_factor;
...
GL1Norm::~GL1Norm() { cudaFree(this->prior); };

float GL1Norm::getNormalizationFactor() { return this->normalization_factor; };
...
void GL1Norm::setEpsilonA(float epsilon) { this->epsilon_a = epsilon; };

void GL1Norm::setEpsilonB(float epsilon) { this->epsilon_b = epsilon; };
...
namespace {
Fi *CreateGL1Norm() { return new GL1Norm; }
