...
std::pair<float, float> BacktrackingArmijo::search(
  ...
  if (gradient == nullptr) {
    std::cerr << "ERROR: BacktrackingArmijo requires gradient but ObjectiveFunction::getCurrentGradient() returned nullptr!" << std::endl;
  ...
  float dir_deriv = deviceReduce<float>(dot_result, dot_elems, threads_bb.x * threads_bb.y);
...
namespace {
LineSearcher* CreateBacktrackingArmijo() {
  return new BacktrackingArmijo();
}

const std::string name = "BacktrackingArmijo";
const bool RegisteredBacktrackingArmijo =
    registerCreationFunction<LineSearcher, std::string>(name, CreateBacktrackingArmijo);
}  // namespace
