...
std::pair<float, float> GLLArmijo::search(float* current_point,
                                          ...
                                          float* mask) {
  // GLL (Grippo-Lampariello-Lucidi) nonmonotone Armijo line search
  static std::deque<float> function_history;  // Store recent function values
  ...
  if (gradient == nullptr) {
    std::cerr << "ERROR: GLLArmijo requires gradient but ObjectiveFunction::getCurrentGradient() returned nullptr!" << std::endl;
  ...
  size_t dot_size = sizeof(float) * M_local * N_local * image_count_local;
  checkCudaErrors(cudaMalloc((void**)&dot_result, dot_size));
  checkCudaErrors(cudaMemset(dot_result, 0, dot_size));
  
  for (int i = 0; i < image_count_local; i++) {
    computeDotProduct<<<numBlocksNN_local, threadsPerBlockNN_local>>>(
  ...
  // Compute reference function value (max of recent M values)
  float f_ref = f0;
  if (!function_history.empty()) {
    f_ref = *std::max_element(function_history.begin(), function_history.end());
  }
  ...
  for (int iter = 0; iter < max_iterations; iter++) {
    ...
    // GLL nonmonotone Armijo condition: f(x + αd) ≤ f_ref + c*α*∇f(x)^T*d
    if (f_alpha <= f_ref + c * alpha * dir_deriv) {
      ...
      function_history.push_back(f_alpha);
      if (function_history.size() > static_cast<size_t>(M_local)) {
        function_history.pop_front();
      }
  ...
  alpha = 1.0f * std::pow(rho, max_iterations - 1);
  ...
  // Note: This will be updated by the optimizer after line search completes
  updateHistory(objective_function, current_point, gradient, alpha);
...
namespace {
LineSearcher* CreateGLLArmijo() {
  return new GLLArmijo();
}

const std::string name = "GLLArmijo";
const bool RegisteredGLLArmijo =
    registerCreationFunction<LineSearcher, std::string>(name, CreateGLLArmijo);
}  // namespace
