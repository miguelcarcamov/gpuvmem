...
__host__ Sinc2D::Sinc2D(int m, int n, Io *imageHandler)
    : CKernel(m, n, imageHandler) {
  this->w = 1.0f;
  this->nameSelf();
};

__host__ Sinc2D::Sinc2D(int m, int n, Io *imageHandler, CKernel *gcf)
    : CKernel(m, n, imageHandler, gcf) {
  this->w = 1.0f;
  this->nameSelf();
};

__host__ Sinc2D::Sinc2D(int m, int n, float dx, float dy)
    : CKernel(m, n, dx, dy) {
  this->w = 1.0f;
  this->nameSelf();
};

__host__ Sinc2D::Sinc2D(int m, int n, float dx, float dy, CKernel *gcf)
    : CKernel(m, n, dx, dy, gcf) {
  this->w = 1.0f;
  this->nameSelf();
};

__host__ Sinc2D::Sinc2D(int m, int n, float dx, float dy, Io *imageHandler)
...
__host__ Sinc2D::Sinc2D(int m, int n, float w, CKernel *gcf)
    : CKernel(m, n, w, gcf) {
  this->nameSelf();
};

__host__ Sinc2D::Sinc2D(int m, int n, float w, Io *imageHandler, CKernel *gcf)
    : CKernel(m, n, w, imageHandler, gcf) {
  this->nameSelf();
};

__host__ Sinc2D::Sinc2D(int m, int n, float dx, float dy, float w)
    : CKernel(m, n, dx, dy, w) {
  this->nameSelf();
};

__host__ Sinc2D::Sinc2D(int m, int n, float dx, float dy, float w, CKernel *gcf)
...
__host__ void Sinc2D::buildKernel(float amp, float x0, float y0, float sigma_x,
                                  float sigma_y) {
  ...
  for (int i = 0; i < this->m; i++) {
    for (int j = 0; j < this->n; j++) {
      ...
      this->kernel[this->n * i + j] =
          sinc2D(amp, x, y, x0, y0, sigma_x, sigma_y, this->w);
    }
  }
...
__host__ void Sinc2D::buildKernel() {
  ...
  for (int i = 0; i < this->m; i++) {
    for (int j = 0; j < this->n; j++) {
      ...
      x = (j - this->support_x) * sigma_x;
      this->kernel[this->n * i + j] =
          sinc2D(this->amp, x, y, this->x0, this->y0, this->sigma_x,
                 this->sigma_y, this->w);
    }
  }
  this->copyKerneltoGPU();
...
__host__ float Sinc2D::GCF(float amp, float x, float y, float x0, float y0,
  ...
  return GCF_fn(amp, distance_x, w * sigma_x) *
         GCF_fn(amp, distance_y, w * sigma_y);
};

__host__ CKernel *Sinc2D::clone() const { return new Sinc2D(*this); };

__host__ void Sinc2D::nameSelf() { this->name = "Sinc"; };

namespace {
CKernel *CreateCKernel() { return new Sinc2D; }

const std::string name = "Sinc2D";
const bool RegisteredSinc2D =
