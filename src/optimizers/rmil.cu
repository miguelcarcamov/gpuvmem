...
float RMIL::computeConjugateGradientParameter(
  ...
  // β_k^{RMIL} = (g_{k+1}^T y_k) / (d_k^T (d_k - g_k)), where y_k = g_{k+1} - g_k
  ...
  for (int i = 0; i < image_count_local; i++) {
    computeDotProduct<<<nblk, tpb>>>(
        device_gg_vector, grad, device_dgg_vector, N_local, M_local, i);
    checkCudaErrors(cudaDeviceSynchronize());
  }
  ...
  for (int i = 0; i < image_count_local; i++) {
    computeDotProduct<<<nblk, tpb>>>(
        device_dgg_vector, dir_prev, device_gg_vector, N_local, M_local, i);
    checkCudaErrors(cudaDeviceSynchronize());
  }
  float denominator = deviceReduce<float>(
      device_dgg_vector, M_local * N_local * image_count_local,
      tpb.x * tpb.y);
...
namespace {
Optimizer* CreateRMIL() {
  return new RMIL;
}

const std::string name_rmil = "CG-RMIL";
const bool RegisteredRMIL =
    registerCreationFunction<Optimizer, std::string>(name_rmil, CreateRMIL);
}  // namespace
