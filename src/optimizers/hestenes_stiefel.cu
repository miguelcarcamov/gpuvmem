...
float HestenesStiefel::computeConjugateGradientParameter(
...
namespace {
Optimizer* CreateHestenesStiefel() {
  return new HestenesStiefel;
}

const std::string name_hs = "CG-HestenesStiefel";
const bool RegisteredHS =
    registerCreationFunction<Optimizer, std::string>(name_hs, CreateHestenesStiefel);
}  // namespace
