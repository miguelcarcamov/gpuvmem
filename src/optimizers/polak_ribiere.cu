...
float PolakRibiere::computeConjugateGradientParameter(
...
namespace {
Optimizer* CreatePolakRibiere() {
  return new PolakRibiere;
}

const std::string name_pr = "CG-PolakRibiere";
const bool RegisteredPR =
    registerCreationFunction<Optimizer, std::string>(name_pr, CreatePolakRibiere);
}  // namespace
