...
__host__ T deviceReduce(T *in, long N, int input_threads)
{
        ...
        if (isPow2(N))
        {
                ...
                {
                case 512:
                        reduceSumKernel<T, 512, true><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case 256:
                        reduceSumKernel<T, 256, true><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case 128:
                        reduceSumKernel<T, 128, true><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case 64:
                        reduceSumKernel<T,  64, true><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case 32:
                        reduceSumKernel<T,  32, true><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case 16:
                        reduceSumKernel<T,  16, true><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case  8:
                        reduceSumKernel<T,   8, true><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case  4:
                        reduceSumKernel<T,   4, true><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case  2:
                        reduceSumKernel<T,   2, true><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case  1:
                        reduceSumKernel<T,   1, true><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;
                }
        }
        else
        {
                ...
                {
                case 512:
                        reduceSumKernel<T, 512, false><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case 256:
                        reduceSumKernel<T, 256, false><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case 128:
                        reduceSumKernel<T, 128, false><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case 64:
                        reduceSumKernel<T,  64, false><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case 32:
                        reduceSumKernel<T,  32, false><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case 16:
                        reduceSumKernel<T,  16, false><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case  8:
                        reduceSumKernel<T,   8, false><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case  4:
                        reduceSumKernel<T,   4, false><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case  2:
                        reduceSumKernel<T,   2, false><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;

                case  1:
                        reduceSumKernel<T,   1, false><<< dimGrid, dimBlock, smemSize >>>(in, d_odata, N);
                        break;
                }
        }
                                ...
                                for(int s=0; s<datasets[d].data.nstokes; s++) {
                                        ...
                                        if(blockSizeV == -1) {
                                                ...
                                                if(UVpow2!=0) {
                                                        ...
                                                        datasets[d].fields[f].device_visibilities[i][s].threadsPerBlockUV = threads1D;
                                                        datasets[d].fields[f].device_visibilities[i][s].numBlocksUV = blocks1D;
                                                }
        ...
        for(int f=0; f<data.nfields; f++) {
          ...
          for (int i = 0; i < data.total_frequencies; i++)
          {
            ...
            for(int s=0; s<data.nstokes; s++) {
              ...
              griddedTogrid(gridded_visibilities[gpu_idx], fields[f].visibilities[i][s].Vm,
              ...
              */
              ...
              checkCudaErrors(cudaMemcpy(vars_gpu[gpu_idx].device_V, gridded_visibilities[gpu_idx].data(),
              ...
              vis_mod2 <<< fields[f].device_visibilities[i][s].numBlocksUV,
                      fields[f].device_visibilities[i][s].threadsPerBlockUV >>>
              (fields[f].device_visibilities[i][s].Vm, vars_gpu[gpu_idx].device_V, fields[f].device_visibilities[i][s].uvw, fields[f].device_visibilities[i][s].weight, deltau, deltav, fields[f].numVisibilitiesPerFreqPerStoke[i][s], N);
              //degriddingGPU<<< fields[f].device_visibilities[i][s].numBlocksUV,
              //            fields[f].device_visibilities[i][s].threadsPerBlockUV >>>(fields[f].device_visibilities[i][s].uvw, fields[f].device_visibilities[i][s].Vm, vars_gpu[gpu_idx].device_V, ckernel->getGPUKernel(), deltau, deltav, fields[f].numVisibilitiesPerFreqPerStoke[i][s], M, N, ckernel->getm(), ckernel->getn(), ckernel->getSupportX(), ckernel->getSupportY());
              checkCudaErrors(cudaDeviceSynchronize());
...
__device__ float calculateL1norm(float *I, float epsilon, float noise, float noise_cut, int index, int M, int N)
{
        const int j = threadIdx.x + blockDim.x * blockIdx.x;
        const int i = threadIdx.y + blockDim.y * blockIdx.y;
...
__global__ void L1Vector(float *L1, float *noise, float *I, long N, long M, float epsilon, float noise_cut, int index)
{
        const int j = threadIdx.x + blockDim.x * blockIdx.x;
        const int i = threadIdx.y + blockDim.y * blockIdx.y;

        L1[N*i+j] = calculateL1norm(I, epsilon, noise[N*i+j], noise_cut, index, M, N);
}

__device__ float calculateDNormL1(float *I, float lambda, float noise, float epsilon, float noise_cut, int index, int M, int N)
{
        const int j = threadIdx.x + blockDim.x * blockIdx.x;
        const int i = threadIdx.y + blockDim.y * blockIdx.y;
        float den;
...
__global__ void DL1NormK(float *dL1, float *I, float *noise, float epsilon, float noise_cut, float lambda, long N, long M, int index)
{
        const int j = threadIdx.x + blockDim.x * blockIdx.x;
        const int i = threadIdx.y + blockDim.y * blockIdx.y;
                                                ...
                                                if (datasets[d].fields[f].numVisibilitiesPerFreqPerStoke[i][s] > 0) {
                                                        ...
                                                        #pragma omp critical
                                                        {
                                                                if (flag_opt % 2 == 0)
                                                                        ...
                                                                        (device_noise_image, result_dchi2, vars_gpu[gpu_idx].device_dchi2, I, datasets[d].fields[f].nu[i], nu_0, noise_cut, fg_scale, threshold, N, M);
                                                                else
...
__host__ float L1Norm(float *I, float * ds, float penalization_factor, float epsilon, int mod, int order, int index, int iter)
{
        ...
        {
                L1Vector<<<numBlocksNN, threadsPerBlockNN>>>(ds, device_noise_image, I, N, M, epsilon, noise_cut, index);
                checkCudaErrors(cudaDeviceSynchronize());
...
__host__ void DL1Norm(float *I, float *dgi, float penalization_factor, float epsilon, int mod, int order, int index, int iter)
{
        ...
        {
                if(flag_opt%2 == index) {
                        DL1NormK<<<numBlocksNN, threadsPerBlockNN>>>(dgi, I, device_noise_image, epsilon, noise_cut, penalization_factor, N, M, index);
                        checkCudaErrors(cudaDeviceSynchronize());
                }
        }
};
