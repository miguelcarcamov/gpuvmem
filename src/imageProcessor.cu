...
void ImageProcessor::apply_beam(cufftComplex *image, float antenna_diameter, float pb_factor, float pb_cutoff, float xobs, float yobs, float freq, beamPtr beam_function)
{
        if(image_count == 2)
                linkApplyBeam2I(image, antenna_diameter, pb_factor, pb_cutoff, xobs, yobs, freq, beam_function);
};
