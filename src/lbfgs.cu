...
__host__ void LBFGS::deallocateMemoryGpu(){FREEALL};
...
__host__ void LBFGS::optimize() {
  ...
  for (int i = 1; i <= this->total_iterations; i++) {
    ...
    this->max_per_it =
        std::max(this->max_per_it,
                 deviceMaxReduce(norm_vector, M * N * image->getImageCount(),
    ...
    for (int i = 0; i < image->getImageCount(); i++) {
      calculateSandY<<<numBlocksNN, threadsPerBlockNN>>>(
          ...
          (this->current_iteration - 1) % this->K, M, N, i);
      checkCudaErrors(cudaDeviceSynchronize());
    }

    LBFGS_recursion(d_y, d_s, xi, std::min(this->K, this->current_iteration),
                    (this->current_iteration - 1) % this->K, M, N);
...
__host__ void LBFGS::LBFGS_recursion(float* d_y,
  ...
  checkCudaErrors(
      cudaMemset(d_r, 0, sizeof(float) * M * N * image->getImageCount()));
  checkCudaErrors(cudaMemcpy(d_q, xi,
                             sizeof(float) * M * N * image->getImageCount(),
                             cudaMemcpyDeviceToDevice));
  ...
  for (int i = 0; i < I->getImageCount(); i++) {
    ...
    for (int k = 0; k < par_M; k++) {
      ...
      beta =
          rho * deviceReduce<float>(aux_vector, M * N,
                                    threadsPerBlockNN.x * threadsPerBlockNN.y);
      // printf("2. image %d - iter %d - rho: %f, alpha: %f, beta: %f,
  ...
  for (int i = 0; i < image->getImageCount(); i++) {
    searchDirection_LBFGS<<<numBlocksNN, threadsPerBlockNN>>>(
        d_r, M, N, i);  // Search direction
    checkCudaErrors(cudaDeviceSynchronize());
  }

  checkCudaErrors(cudaMemcpy(xi, d_r,
                             sizeof(float) * M * N * image->getImageCount(),
                             cudaMemcpyDeviceToDevice));
  cudaFree(aux_vector);
  cudaFree(d_q);
