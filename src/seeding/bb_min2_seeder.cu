...
#include "seeding/bb_min2_seeder.cuh"
...
float BBMin2Seeder::seed(ObjectiveFunction* objective_function,
  ...
  // Barzilai-Borwein Min2: α = max(α_BB1, α_BB2)
  // Same computation as BBMin1, but return maximum
  ...
  if (alpha_bb1 > 0.0f && alpha_bb2 > 0.0f) {
    return std::max(alpha_bb1, alpha_bb2);
...
namespace {
StepSizeSeeder* CreateBBMin2Seeder() {
  return new BBMin2Seeder();
}

const std::string name = "BBMin2Seeder";
const bool RegisteredBBMin2Seeder =
    registerCreationFunction<StepSizeSeeder, std::string>(name, CreateBBMin2Seeder);
}  // namespace
