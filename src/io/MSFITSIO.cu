...
// Explicit template instantiations for types actually used
template __host__ float readHeaderKeyword<float>(char* filename, char* keyword, int type);
template __host__ headerValues open_fits<float>(float** data, const char* filename, int datatype);
template __host__ headerValues open_fits<double>(double** data, const char* filename, int datatype);
template __host__ headerValues open_fits<int>(int** data, const char* filename, int datatype);
