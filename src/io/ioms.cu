...
// Copy a Measurement Set from source to destination
static void MScopy(const char* in_dir, const char* in_dir_dest) {
  std::string dir_origin = in_dir;
  std::string dir_dest = in_dir_dest;
