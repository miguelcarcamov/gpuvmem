...
void IoMS::writeSpecificColumn(std::vector<Field>& fields, MSData data) {
  writeMS(this->output.c_str(), this->datacolumn_output.c_str(), fields, data,
...
void IoMS::writeModelVisibilities(std::vector<Field>& fields, MSData data) {
  writeMS(this->output.c_str(), "MODEL_DATA", fields, data,
          this->random_probability, true, this->apply_noise_output,
          this->W_projection);
};
