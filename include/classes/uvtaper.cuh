...
class UVTaper {
 public:
  ...
  __host__ float getSigma_maj() { return this->sigma_maj; };

  __host__ float getSigma_min() { return this->sigma_min; };

  __host__ float getBPA() { return this->bpa; };

  __host__ void setAmplitude(float amplitude) { this->amplitude = amplitude; };

  __host__ void setSigma_maj(float sigma_maj) { this->sigma_maj = sigma_maj; };

  __host__ void setSigma_min(float sigma_min) { this->sigma_min = sigma_min; };
  ...
  __host__ void setFWHM_arcsec(float fwhm_maj_sky_arcsec,
                               float fwhm_min_sky_arcsec) {
  ...
  __host__ void setBPA(float bpa) { this->bpa = bpa; };
  ...
  __host__ float getValue(double u, double v) {
    ...
    float a = (cos_bpa * cos_bpa) / (2.0f * this->sigma_maj * this->sigma_maj) +
              (sin_bpa * sin_bpa) / (2.0f * this->sigma_min * this->sigma_min);
    float b = sin_bpa_2 / (2.0f * this->sigma_maj * this->sigma_maj) -
              sin_bpa_2 / (2.0f * this->sigma_min * this->sigma_min);
    float c = (sin_bpa * sin_bpa) / (2.0f * this->sigma_maj * this->sigma_maj) +
              (cos_bpa * cos_bpa) / (2.0f * this->sigma_min * this->sigma_min);
