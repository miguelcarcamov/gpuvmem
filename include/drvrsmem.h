...
#define SHARED_KEYBASE \
  (14011963) /* base for shared memory keys, may be overriden by getenv */
#define SHARED_FDNAME                                     \
  ("/tmp/.shmem-lockfile") /* template for lock file name \
                            */

#define SHARED_ENV_KEYBASE \
  ("SHMEM_LIB_KEYBASE") /* name of environment variable */
#define SHARED_ENV_MAXSEG \
  ("SHMEM_LIB_MAXSEG") /* name of environment variable */
...
#define SHARED_WAIT \
  (0) /* flag for shared_lock, block if cannot lock immediate */
#define SHARED_NOWAIT \
  (2) /* flag for shared_lock, fail if cannot lock immediate */
...
#define SHARED_PERSIST                                                 \
  (8) /* flag for shared_malloc, object is not deleted after last proc \
         detaches */
...
#define SHARED_GRANUL                                                         \
  (16384) /* granularity of shared_malloc allocation = phys page size, system \
             dependent */
...
typedef union {
  struct BLKHEADstruct {
    ...
    int handle; /* this is not necessary, used only for non-resizeable objects
                   via ptr */
  } s;
...
typedef struct SHARED_GTABstruct /* data type used in global table */
{
  ...
  int key; /* key value used to generate shared memory handle (realloc changes
              it) */
...
SHARED_P shared_realloc(int idx,
                        long newsize); /* reallocate n-bytes of shared memory
                                          (ON LOCKED SEGMENT ONLY) */
int shared_size(int idx); /* get size of attached shared memory segment (ON
                             LOCKED SEGMENT ONLY) */
int shared_attr(int idx); /* get attributes of attached shared memory segment
                             (ON LOCKED SEGMENT ONLY) */
int shared_set_attr(int idx,
                    int newattr); /* set attributes of attached shared memory
                                     segment (ON LOCKED SEGMENT ONLY) */
