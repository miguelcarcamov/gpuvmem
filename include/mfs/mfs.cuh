...
class MFS : public Synthesizer {
 ...
 private:
  ...
  std::unique_ptr<gpuvmem::cli::IRunObserver> run_observer_;
  ...
  void cli_diagnostic(gpuvmem::cli::DiagnosticLevel level, const std::string& message) const;
  gpuvmem::cli::RunSummary build_run_summary() const;
  void emit_run_summary() const;
};
