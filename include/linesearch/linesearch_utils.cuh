 ...
 *
 * Computes p = p + α*d (linear), then applies `Projection` per plane (Pyralysis-style).
 *
 ...
 */
__host__ void updatePoint(ObjectiveFunction* objective_function, Image* image, float* p, float* d,
                          float alpha);
...
// Host wrappers for line search kernels (linear trial; projection applied separately).
...
/** Apply active `LineSearcher` projection to one image plane (see `ProjectionApplyContext`). */
